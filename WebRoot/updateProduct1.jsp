...
<jsp:useBean id="Product" class="daolmpl.Productlmpl" scope="application"></jsp:useBean>
 ...
 <br>
 	...
 	<div class="card">
 		<div class= "card-header bg-primary">
 			<h4 class="card-title text-while">内容修改</h4>
 		</div>
 		<div class="card-body">
 			<form action="ProductServlet?type=update2" role="form" method="post" enctype="multipart/form-data" id="updatenews" name="updatenews">
 				<input type="hidden" name="Pid" id="Pid" value="${product.id }">
 				<div class="form-group">
 					<label for="title">标题：</label>
 					<input class="form-control" type="text" value="${product.name }"  id="title" name="title" placeholder="请输入标题">
 				</div>
 				<div class="form-group">
 					<label for="Ctype">新闻类型</label>
 					<select class="form-control" id="Ctype" name = "Ctype">
 					<c:forEach var="row" items="${Ptypelist }">
 					<option><c:out value="${row.type }"></c:out></option>
 					</c:forEach>
 					</select>
 				</div>	
 				<div class="form-group">
 					<label for="txt">内容：</label>
 					<textarea rows="3" cols="12" class="form-control" id="content" name="content" placeholder="请输入内容">${product.introduce }</textarea>
 				</div>
